...
At the moment we got a dumb webpage and what we need to do is to look for a way to delegate the authentication process to RHSSO, after all we don't want our service to know anything about this, so we do directing our user via that **login** to an especific keycloak endpoint called ``authorization_endpoint`` (you can find this endpoint in the discovery payload from above). 
...
- **client_id** - You need to [register a client in RHSSO](https://www.keycloak.org/docs/latest/getting_started/keycloak-images/add-client.png), usually the name of the client is the client ID.
